...
double a_to_b(double** A, double** B, int i,int j, int k)
{
	callsOfDifference++;
	callsOfCom++;
	if (1 - A[i][k] > B[k][j])
...
double b_to_a(double** A, double** B,int i, int j, int k)
{
	callsOfDifference++;
	callsOfCom++;
	if (1 - B[k][j] > A[i][k])
...
void Fijk(double** A, double** B, double* E, int i, int j, int k)
{
	...
	callsOfDifference += 3;
	callsOfSumm += 2;
	F[i][j][k] = (a_to_b(A, B,i, j, k) * (2. * E[k] - 1.) * E[k] + b_to_a(A, B, i,j,k) * (1. + (4. * a_to_b(A, B, i,j, k) - 2.) * E[k]) * (1. - E[k]));
}
